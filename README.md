...
4. **Verdict** — each fault ends up in one of three states:
   ...
   - **Aborted** — the per-fault search budget (backtrack count + a wall-clock cap) was exceeded before a verdict was reached. This is kept strictly separate from "redundant" so an inconclusive result never gets reported as a proof.
